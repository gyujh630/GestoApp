...
function refactorCoordinate(x, y, z) {
  ...
  function areTipsTouching(landmarks, fingerNum) {
    ...
    const dist = getDistance(thumb_tip[0], thumb_tip[1], thumb_tip[2], other_tip[0], other_tip[1], other_tip[2]);
    if (dist <= 5) {
      return true;
    }
    else return false;
  ...
  //   let dip = refactorCoordinate(landmarks[4 * fingerNum + 3].x, landmarks[4 * fingerNum + 3].y, landmarks[4 * fingerNum + 3].z);
  //   let tip = refactorCoordinate(landmarks[4 * fingerNum + 4].x, landmarks[4 * fingerNum + 4].y, landmarks[4 * fingerNum + 4].z);
  ...
  export function getGesture(landmarks) {
    ...
    if (landmarks.length > 1) {   // 두 손이 인식될 때
    ...
    if (second_hand == undefined) {
      /* HOLD */
      if (
        isFingerFold(first_hand, 3, 30, 60, 20) &&
        isFingerFold(first_hand, 4, 30, 60, 20) &&
        isFingerFold(first_hand, 2, 30, 60, 20) &&
        areTipsTouching(first_hand, 1)
      ) {
        return "HOLD";
      }
      ...
      if (
        ...
        (isFingerFold(first_hand, 3, 0, 90, 0) ||
          isFingerFold(first_hand, 3, 90, 0, 0) ||
          isFingerFold(first_hand, 4, 0, 90, 0) ||
          isFingerFold(first_hand, 4, 90, 0, 0))
      ) {
        return "POINTER";
      }
      return "???";
